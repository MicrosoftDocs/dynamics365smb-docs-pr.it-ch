...
La maggior parte dei giornali si basa sul giornale generale ed è possibile elaborare tutte le transazioni nella pagina  **Giornali generali** . Ulteriori informazioni in [Registrare le transazioni direttamente nella contabilità generale](finance-how-post-transactions-directly.md).  
...
Tuttavia, [!INCLUDE [prod_short](includes/prod_short.md)] offre anche registri ottimizzati per specifici tipi di transazioni, come il *giornale dei pagamenti* per la registrazione dei pagamenti. Per ulteriori informazioni, vedi [Registrare pagamenti e resi nelle Registrazioni pagamenti](payables-how-post-payments-refunds.md).  
...
## <a name="add-context-to-general-journal-transactions"></a>Aggiungere contesto alle transazioni delle registrazioni COGE
...
Esistono numerose definizioni registrazioni COGE. Ogni modello di giornale è rappresentato da una pagina dedicata con funzioni specifiche e i campi necessari per supportare tali funzioni, come la pagina  **Giornali di riconciliazione dei pagamenti** per elaborare i pagamenti bancari e la pagina  **Giornali di pagamento** per pagare i fornitori o rimborsare i dipendenti. Per ulteriori informazioni, vedi [Effettuare i pagamenti](payables-make-payments.md) e [Riconciliare i pagamenti clienti con la registrazione incassi o da movimenti contabili clienti](receivables-how-apply-sales-transactions-manually.md).
...
Puoi abilitare un controllo in background che ti aiuterà a prevenire ritardi durante la registrazione. Il controllo ti avviserà quando un errore nel giornale di registrazione finanziario su cui stai lavorando ti impedirà di pubblicare la registrazione. Nella pagina  **Batch di giornale di registrazione generale**, puoi scegliere  **Controllo errori in background** per  [!INCLUDE[prod_short](includes/prod_short.md)] convalidare i giornali finanziari, come i giornali generali o di pagamento, mentre ci stai lavorando.

Quando si abilita la convalida, le schede dettaglio **Controlla documento** o **Verifica registrazione** mostrerà i problemi nella riga corrente e nell'intero batch. La convalida si verifica quando si carica un batch registrazioni COGE e quando si sceglie un'altra riga di registrazione. Il riquadro **Problemi totali** nella scheda dettaglio mostra il numero totale di problemi rilevati da [!INCLUDE[prod_short](includes/prod_short.md)] ed è possibile selezionarlo per aprire una panoramica dei problemi.
...
Il campo **Metodo ricorrenza** è importante. Determina come trattare l'importo nella riga delle registrazioni dopo la registrazione. Se ad esempio utilizzi il medesimo importo per ogni registrazione, l'importo risulta invariato. Se nella riga vengono utilizzati i medesimi conti e testo e l'importo varia a ogni registrazione, sarà possibile eliminare l'importo dopo la registrazione.

| A | Vedere |
| --- | --- |
|C Costante|Dopo la contabilizzazione l'importo specificato nella riga di registrazione rimane invariato. Le righe con importi pari a zero rimangono nel giornale di registrazione, ma non vengono registrate. È possibile aggiornare l'importo in un'esecuzione successiva.  |
|V Variabile|Dopo la contabilizzazione l'importo specificato nella riga di registrazione viene eliminato.|
...
Se si desidera registrare un movimento l'ultimo giorno di ogni mese, è possibile eseguire una delle azioni seguenti:
...
In questo campo viene visualizzata la data in cui la riga viene registrata per l'ultima volta. Oltre tale data, la riga non verrà più registrata.
...
Se il campo rimane vuoto, la riga viene registrata ogni volta fino a quando non viene eliminata dalle registrazioni.
...
Nella pagina  **Giornali generali ricorrenti**, è possibile scegliere l'azione  **Assegnazioni** per specificare come allocare gli importi sulla riga del giornale ricorrente a più conti e dimensioni. L'allocazione è una riga delle registrazioni periodiche per la riga di contropartita.
...
> [!NOTE]
> Non è possibile utilizzare conti di allocazione in batch di registrazioni che dispongono di un flusso di lavoro di approvazione per la creazione di righe di registrazione.
...
### <a name="calculate-the-reversal-date"></a>Calcolare la data storno
...
1. Scegli la ![lampadina che apre la funzione Dimmi.](media/ui-search/search_small.png "Informazioni sull'operazione che si desidera eseguire") immetti **Registrazioni articoli**, quindi scegli il collegamento correlato.
...
La rinumerazione dei documenti rispetta i collegamenti correlati, ad esempio il collegamento di un pagamento tra il documento nella riga registrazioni e un conto fornitore. Di conseguenza, i campi **Collega-a ID** e **Collega-a nr. doc.** verranno aggiornati nei movimenti contabili fornitori.

### <a name="to-renumber-documents-in-journals"></a>Per rinumerare i documenti nei giornali di registrazione

La seguente procedura si basa sulla pagina  **Giornali generali**, ma si applica a tutti gli altri giornali basati sul giornale generale, come la pagina  **Giornali dei pagamenti** .
