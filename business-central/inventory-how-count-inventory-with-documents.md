---
title: Conta e Regola inventario
...
---
# Conteggio e rettifica dell'inventario utilizzando documenti
...
1. Scegli la ![lampadina che apre la funzione Dimmi.](media/ui-search/search_small.png "Informazioni sull'operazione che si desidera eseguire") immetti **Ordini inventario fisico**, quindi scegli il collegamento correlato.
...
4. Nella scheda **Azioni**, nel gruppo **Funzioni**, scegli **Altro**, quindi scegli **Calcola linee**.
...
1. In una pagina **Ordine di inventario fisico** che contiene righe per gli articoli da conteggiare in una registrazione, è necessario scegliere l'azione **Crea nuova registrazione** .
1. Nella scheda **Azioni**, nel gruppo **Funzioni**, scegli **Altro**, quindi scegli **Crea nuova registrazione**.
1. Selezionare le opzioni e impostare i filtri come necessario.
1. Scegliere il pulsante **OK**.
1. Carica ogni set di articoli da conteggiare nell'ordine di inventario fisico correlato e ripeti i passaggi da 1 a 3 con la casella di controllo **Solo righe non nelle registrazioni** selezionata.
1. Nella scheda  **Correlati**, seleziona  **Ordine**, quindi seleziona l'azione  **Registrazioni** per aprire la pagina  **Elenco registrazioni inventario fisico** .
1. Aprire la registrazione pertinente.
1. Compilare i campi appropriati nella Scheda dettaglio **Generale**.
1. Per gli articoli che utilizzano la tracciabilità degli articoli, creare una riga supplementare per ogni codice di numero di lotto o numero seriale scegliendo l'azione **Funzioni** e quindi l'azione **Copia riga**. Per saperne di più, vedi [Gestire la tracciabilità degli articoli durante il conteggio dell'inventario](#handle-item-tracking-when-counting-inventory).  
1. Scegli l'azione **Stampa** per preparare il documento fisico che i dipendenti possono utilizzare per annotare le quantità che conteggiano.
...
Dopo aver completato una registrazione dell'inventario fisico, il campo  **Registratore quantità (base)** nell'ordine di inventario fisico correlato viene aggiornato con i valori conteggiati (registrati) e la casella di controllo  **Sulle righe di registrazione** viene selezionata. Se una quantità conteggiata differisce dalla quantità prevista, i campi  **Pos Qty. (Base)** e  **Neg Qty. (Base)** mostrano la differenza.
...
1. Nella scheda  **Correlati**, seleziona  **Altro**, quindi seleziona  **Mostra linee duplicate**.
...
3. Nella scheda  **Correlati**, seleziona  **Ordine**, quindi seleziona l'azione  **Registrazioni** per visualizzare un elenco delle registrazioni di inventario fisico correlate.  

## Gestire la tracciabilità degli articoli durante il conteggio dell'inventario
...
### Esempio - Registrare le differenze nei numeri di lotto conteggiati
...
È possibile stampare questi documenti in qualsiasi momento, rilasciarli, riaprirli e assegnare valori comuni, come le dimensioni, nell'intestazione. Per ristampare i documenti dopo la registrazione, utilizza le pagine **Ricevuta magazzino registrata** e **Spedizione magazzino registrata**.
...
2. Nella Scheda dettaglio **Numerazione**, specifica nei seguenti campi la serie di numeri per i documenti:
   ...
   - **Numero di ricevuta di inv.**  
   - **Numero di ricevuta di invt. pubblicato.**  
   - **Numeri di spedizione Invt.**  
   - **Numeri di spedizione Invt. pubblicati.**  
...
2. Nell'intestazione della pagina  **Ricevuta di accredito**, seleziona la posizione nel campo  **Codice posizione**, quindi compila i campi rimanenti come necessario.
...
4. Per stampare un report  **Ricevuta inventario** dalla pagina  **Ricevuta inventario**, selezionare l'azione  **Stampa** .

Nella pagina  **Ricevuta di convalida** sono disponibili le seguenti funzioni:
...
[Conteggio, Regola e riclassificazione dell'inventario mediante i giornali](inventory-how-count-adjust-reclassify.md)    
[Lavorare con numeri di serie e di lotto](inventory-how-work-item-tracking.md)    
