...
* Le valute locali utilizzate da ciascuna società. Le società [!INCLUDE [prod_short](includes/prod_short.md)] possono connettersi a un ambiente [!INCLUDE [cds_long_md](includes/cds_long_md.md)] che ha una valuta di base diversa dalla valuta locale. Per ulteriori informazioni su come gestire configurazioni multivaluta, vedi [Consentire valute diverse](#allow-for-different-currencies).
...
> [!NOTE]
> La sincronizzazione di più valute richiede l'utilizzo di una sincronizzazione unidirezionale, da [!INCLUDE [prod_short](includes/prod_short.md)] a [!INCLUDE [cds_long_md](includes/cds_long_md.md)].

Per ulteriori informazioni sulla valuta di base in [!INCLUDE [cds_long_md](includes/cds_long_md.md)], vedi [Entità della valuta (valuta) di transazione](/powerapps/developer/data-platform/transaction-currency-currency-entity). 

Per ulteriori informazioni sulle valute in [!INCLUDE [prod_short](includes/prod_short.md)], vedi [Valute in Business Central](finance-currencies.md).

Per consentire valute diverse, prima di connetterti, assicurati di aver specificato le seguenti impostazioni:

* L'impostazione della valuta della transazione di base in [!INCLUDE [cds_long_md](includes/cds_long_md.md)] ha il codice valuta specificato nella pagina **Valute** in [!INCLUDE [prod_short](includes/prod_short.md)].
* Esiste almeno un tasso di cambio specificato per la valuta in [!INCLUDE [prod_short](includes/prod_short.md)] nella pagina **Tassi di cambio valuta**.

Quando abiliti la connessione a [!INCLUDE [cds_long_md](includes/cds_long_md.md)], [!INCLUDE [prod_short](includes/prod_short.md)] aggiunge la valuta locale all'entità **Valuta** in [!INCLUDE [cds_long_md](includes/cds_long_md.md)]. La valuta locale utilizza il tasso di cambio indicato nel campo **Fattore valuta** nella pagina  **Tassi di cambio valuta**.

Poiché la sincronizzazione della valuta è unidirezionale, da [!INCLUDE [prod_short](includes/prod_short.md)] a [!INCLUDE [cds_long_md](includes/cds_long_md.md)], gli importi monetari vengono convertiti e sincronizzati come segue:
...
9. Accedere con l'account amministratore per Microsoft Entra ID (questo account deve avere una licenza valida per [!INCLUDE[cds_long_md](includes/cds_long_md.md)] ed essere un amministratore nell'ambiente [!INCLUDE[cds_long_md](includes/cds_long_md.md)]). Dopo aver effettuato l'accesso, verrà richiesto di consentire all'applicazione registrata l'accesso a [!INCLUDE[cds_long_md](includes/cds_long_md.md)] per conto dell'organizzazione. È necessario fornire il consenso per completare il setup.
