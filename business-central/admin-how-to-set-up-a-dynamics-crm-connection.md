...
> [!Important]
> Durante il setup della connessione a [!INCLUDE[d365fin](includes/cds_long_md.md)], all'amministratore verrà chiesto di concedere le autorizzazioni seguenti per l'applicazione Azure registrata denominata [!INCLUDE[d365fin](includes/d365fin_md.md)] Integration a [!INCLUDE[d365fin](includes/cds_long_md.md)]:
> * L'autorizzazione **Accesso [!INCLUDE[d365fin](includes/cds_long_md.md)] come utente corrente** è necessaria in modo che [!INCLUDE[d365fin](includes/d365fin_md.md)] possa per conto dell'amministratore, creare automaticamente un utente dell'applicazione [!INCLUDE[d365fin](includes/d365fin_md.md)] Integration senza licenza non interattivo, assegnare ruoli di sicurezza a questo utente e distribuire la soluzione di integrazione CDS di base [!INCLUDE[d365fin](includes/d365fin_md.md)] in [!INCLUDE[d365fin](includes/cds_long_md.md)]. Questa autorizzazione viene utilizzata una sola volta durante il setup della connessione a [!INCLUDE[d365fin](includes/cds_long_md.md)]. 
...
> * L'autorizzazione **Accedi e leggi il profilo** è necessaria per verificare che l'accesso dell'utente abbia effettivamente assegnato il ruolo di sicurezza Amministratore di sistema in [!INCLUDE[d365fin](includes/cds_long_md.md)]. 
>
> Fornendo il consenso per conto dell'organizzazione, l'amministratore autorizza l'applicazione Azure registrata chiamata [!INCLUDE[d365fin](includes/d365fin_md.md)] Integration per [!INCLUDE[d365fin](includes/cds_long_md.md)] a sincronizzare i dati usando le credenziali dell'utente dell'applicazione [!INCLUDE[d365fin](includes/d365fin_md.md)] Integration creato automaticamente.
...
Per connettere [!INCLUDE[d365fin](includes/d365fin_md.md)] on-premises a [!INCLUDE[d365fin](includes/cds_long_md.md)], è necessario specificare alcune informazioni nella pagina **Setup connessione a Common Data Service**.
...
I seguenti passaggi presuppongono che si stia utilizzando Azure AD per gestire identità e accesso. Per ulteriori informazioni sulla registrazione di un'applicazione in Azure AD vedere [Avvio rapido: registrare un'applicazione con la piattaforma di identità Microsoft](https://docs.microsoft.com/azure/active-directory/develop/quickstart-register-app). Se non si utilizza Azure AD vedere [Utilizzo di un altro servizio di gestione dell'identità e degli accessi](admin-how-to-set-up-a-dynamics-crm-connection.md#using-another-identity-and-access-management-service). 
...
7. In [!INCLUDE[d365fin](includes/d365fin_md.md)], nella pagina **Setup connessione a Common Data Service** nel campo **URL ambiente** inserire l'URL per l'ambiente [!INCLUDE[d365fin](includes/cds_long_md.md)].
8. Per abilitare la connessione a [!INCLUDE[d365fin](includes/cds_long_md.md)], attivare l'interruttore **Abilitato**.
9. Accedere con l'account amministratore per Azure Active Directory (questo account deve avere una licenza valida per [!INCLUDE[d365fin](includes/cds_long_md.md)] ed essere un amministratore nell'ambiente [!INCLUDE[d365fin](includes/cds_long_md.md)]). Dopo aver effettuato l'accesso, verrà richiesto di consentire all'applicazione registrata l'accesso a [!INCLUDE[d365fin](includes/cds_long_md.md)] per conto dell'organizzazione. È necessario fornire il consenso per completare il setup.
