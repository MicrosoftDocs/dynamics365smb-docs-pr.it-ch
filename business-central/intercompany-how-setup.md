...
Nella pagina **Setup interaziendale** ciascun partner specifica il partner di sincronizzazione nel campo **Partner di sincronizzazione**. Successivamente, vengono specificati automaticamente il piano dei conti intercompany e le dimensioni intercompany, in base al setup del partner di sincronizzazione. I partner utilizzano quindi le pagine **Mappatura piano dei conti intercompany** e **Mappatura dimensione intercompany** per mappare il proprio piano dei conti e dimensioni al piano dei conti intercompany e alle dimensioni e viceversa. 
...
* Configurare [!INCLUDE [prod_short](includes/prod_short.md)] come app registrata nel portale di Azure.
* Aggiungere e abilitare la registrazione dell'app in [!INCLUDE [prod_short](includes/prod_short.md)].
* Scambiare informazioni sui setup interaziendali. Ciascun partner può ottenere queste informazioni dalla guida al setup assistito **Setup partner IC nell'ambiente**.
...
* Scambiare le seguenti impostazioni di autenticazione in modo che [!INCLUDE [prod_short](includes/prod_short.md)] possa autenticarsi quando viene eseguita la connessione. Ogni partner può ottenere queste informazioni dalla propria app registrata. Per ulteriori informazioni su come creare un'app registrata, vedi [Creare un'app registrata nel portale di Azure](#create-a-registered-app-in-azure-portal).  
...
> [!TIP]
> È una buona idea tenere aperto un editor di testo, ad esempio Blocco note, mentre crei l'app registrata. Avrai bisogno di alcune informazioni quando esegui la guida al setup assistito **Setup partner IC nell'ambiente**, quindi è conveniente avere le informazioni a portata di mano.

1. Vai al [portale di Azure](https://portal.azure.com/#home).
2. Scegli il servizio **ID Microsoft Entra**.
3. Nel riquadro di spostamento sinistro scegli **Registrazioni app**.
4. Nella pagina **Registrazioni app** scegli **nuova registrazione**.
5. Assegna un nome all'applicazione.
6. Scegli il tipo di account **Account in qualsiasi directory organizzativa (qualsiasi tenant Microsoft Entra ID - Multitenant)**.
7. Per l'URI di reindirizzamento, nel campo **Seleziona una piattaforma** , scegli **Web**, quindi fornisci l'URI. Ad esempio, \**https://businesscentral.dynamics.com/OAuthLanding.htm**.

  Se lavori con un ISV incorporato, potresti aver bisogno di un URI diverso.

8. Registra l'app.
9. Nella pagina **App interaziendale**, nel riquadro di spostamento, scegli **Autorizzazioni API**.
10. Scegli l'azione **Aggiungi un'autorizzazione**.
11. Nella scheda **API Microsoft** scegli **Dynamics 365 Business Central**.
12. Nel riquadro **Richiedi autorizzazioni API**, scegli **Autorizzazioni applicazione**, quindi scegli l'autorizzazione **API.ReadWrite.All**.
13. Nella pagina **App interaziendale | Autorizzazioni API**, scegli l'azione **Concedi consenso dell'amministratore per Contoso** e quindi concedi il consenso.
14. Scegli **Certificati e segreti** nel riquadro di spostamento.
15. Scegli la scheda **Segreti client**, quindi scegli **Nuovo segreto client**.
16. Nel riquadro **Aggiungi un segreto client**, immetti un nome per il segreto e specifica quando scadrà.
...
17. Scegli **Panoramica** nel riquadro di spostamento.
18. Copia il valore nel campo **ID applicazione (client)**. Ad esempio, incolla il valore in un editor di testo.
19. Scegli l'azione **Endpoint** , quindi copia il valore nel campo **Endpoint token OAuth 2.0 (v2)** . Ad esempio, incolla il valore in un editor di testo.
...
1. Scegli la ![lampadina che apre la funzione Dimmi.](media/ui-search/search_small.png "Informazioni sull'operazione che si desidera eseguire") immetti **Scheda applicazione Microsoft Entra**, quindi scegli il link correlato.  
2. Compilare i campi in base alle esigenze. [!INCLUDE [tooltip-inline-tip_md](includes/tooltip-inline-tip_md.md)]
3. Nel campo **Stato** scegli **Abilitato**. 
...
1. Scegli la ![lampadina che apre la funzione Dimmi.](media/ui-search/search_small.png "Informazioni sull'operazione che si desidera eseguire") immetti **Setup IC**, quindi scegli il collegamento correlato.
